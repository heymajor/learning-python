...
def remove_capitals( boop ):
    
    boop_array = list( map( str, boop ))
    boop_string = []

    for boops in boop_array:
        if ord(boops) < 65 or ord(boops) > 90:
            boop_string.append(boops)

    return "".join(boop_string)
