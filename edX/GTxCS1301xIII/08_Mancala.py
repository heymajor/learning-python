...
#  Verifies p1 is ready to tally score
    for i in p1[1:]:
        if i > 0:
            counter += 1
            break

# Verifies p2 is ready to tally score
        else:
            for i in p2[:1]:
                if i > 0:
                    counter += 1

# Determines end-message depending on if the game is over and who has a higher score
    if counter > 0:
        end = "Keep playing!"
    else:
        if p1[0] > p2[6]:
